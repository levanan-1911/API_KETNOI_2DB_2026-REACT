...
def get_departments():
    """Lấy danh sách phòng ban từ HUMAN_2025 với số nhân viên"""
    ...
    """)
    rows = [{"DepartmentID": r[0], "DepartmentName": r[1], "EmployeeCount": r[2]} for r in cur.fetchall()]
    return jsonify(rows)


@router.route("/api/departments", methods=["POST"])
def add_department():
    """Thêm phòng ban mới vào HUMAN_2025 và đồng bộ MySQL"""
    ...
    try:
        cur = sql.cursor()
        cur.execute("INSERT INTO Departments (DepartmentName) OUTPUT INSERTED.DepartmentID VALUES (?)", dept_name)
        dept_id = cur.fetchone()[0]

        my_cur = my.cursor()
        my_cur.execute("INSERT INTO departments_payroll (DepartmentID, DepartmentName) VALUES (%s, %s)", (dept_id, dept_name))

        sql.commit()
        my.commit()

        return jsonify({"status": "success", "data": {"DepartmentID": dept_id, "DepartmentName": dept_name}})
...
def update_department(dept_id):
    """Cập nhật phòng ban và đồng bộ MySQL"""
    data = request.get_json()
    dept_name = data.get("DepartmentName", "").strip()
    ...
    try:
        ...
        my_cur = my.cursor()
        my_cur.execute("INSERT INTO departments_payroll (DepartmentID, DepartmentName) VALUES (%s, %s) ON DUPLICATE KEY UPDATE DepartmentName = %s, SyncedAt = CURRENT_TIMESTAMP", (dept_id, dept_name, dept_name))
...
def delete_department(dept_id):
    """Xóa phòng ban và đồng bộ MySQL"""
    sql = get_sqlserver_connection()
    my = get_mysql_connection()
...
def add_position():
    """Thêm chức vụ mới vào HUMAN_2025 và đồng bộ MySQL"""
    data = request.get_json()
    pos_name = data.get("PositionName", "").strip()
    
    if not pos_name:
        return jsonify({"status": "error", "msg": "Tên chức vụ không được để trống"}), 400
    ...
    try:
        cur = sql.cursor()
        cur.execute("INSERT INTO Positions (PositionName) OUTPUT INSERTED.PositionID VALUES (?)", pos_name)
        pos_id = cur.fetchone()[0]

        my_cur = my.cursor()
        my_cur.execute("INSERT INTO positions_payroll (PositionID, PositionName) VALUES (%s, %s)", (pos_id, pos_name))

        sql.commit()
        my.commit()

        return jsonify({"status": "success", "data": {"PositionID": pos_id, "PositionName": pos_name}})
...
def update_position(pos_id):
    """Cập nhật chức vụ và đồng bộ MySQL"""
    data = request.get_json()
    pos_name = data.get("PositionName", "").strip()
    ...
    try:
        ...
        my_cur = my.cursor()
        my_cur.execute("INSERT INTO positions_payroll (PositionID, PositionName) VALUES (%s, %s) ON DUPLICATE KEY UPDATE PositionName = %s, SyncedAt = CURRENT_TIMESTAMP", (pos_id, pos_name, pos_name))
...
def delete_employee(emp_id):
    """
    Xóa nhân viên:
    1. DELETE Dividends (SQL Server)
    2. DELETE Employees (SQL Server)
...
# ============================================================
# REPORTS – Dashboard KPI (gộp 2 DB)
# ============================================================
